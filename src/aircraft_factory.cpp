...
std::unique_ptr<Aircraft> AircraftFactory::create_aircraft(const AircraftType& type, Tower& t) const
{
    ...
    return std::make_unique<Aircraft>(type, flight_number, start, direction, t); 
}

std::unique_ptr<Aircraft> AircraftFactory::create_random_aircraft(Tower& t) const
{
    return create_aircraft(aircraft_types[rand() % 3], t);
}

void AircraftFactory::print_number_aircraft(int n) const
{
